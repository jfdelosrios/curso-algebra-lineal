...
Los elementos estrictamente superiores de una matriz son los que se encuentran por encima de la diagonal
principal
...
$A_{i,j} i < j$ está en la diagonal principal

$A_{i,j} i = j$ está por encima de la diagonal principal

$A_{i,j} i > j$ está por debajo de la diagonal principal
...
$$
AB_{12,3} = 
a_{12,1}b_{13} + a_{12,2}b_{23} + \cdot \cdot \cdot + a_{12,15}b_{15,3} =
\sum_{k=1}^{15} a_{12,k}b_{k3}
$$
...
$$
AB_{ij} = + + \cdot \cdot \cdot + + = \sum_{k=1}^{n}
$$
...
Supongamos ahora que sabemos el valor de los $s_k$, Por ejemplo, $s_k = 7$ $\forall k \in \{1, . . . , 6\}$ y $s_k = 5$ $\forall k \in \{7, . . . , 20\}$. Entonces
...
$$\sum_{k=1}^{17} s_k = \cdot \cdot \cdot$$ donde $s_k = 2$ $\forall k \in \{1, . . . , 10\}, s_k = 3$ $\forall k \in \{11, . . . , 17\}$

$$\sum_{k=1}^{25} s_k= \cdot \cdot \cdot$$ donde $s_k = 7$ $\forall k \in \{1, . . . , 12\}, s_k = 9$ $\forall k \in \{13, . . . , 19\}, s_k = 5$ $\forall k \in \{20, . . . , 25\}$

Simplifica las sumas:

$\sum_{k=1}^{31} s_k = \cdot \cdot \cdot$ donde $s_k = 0$ $\forall k \in \{1, . . . , 9\} \cup \{25, . . . , 31\}$

$\sum_{k=1}^{27} s_k = \cdot \cdot \cdot$ donde $s_k = 0$ $\forall k \in \{1, . . . , 7\} \cup \{12, \cdot \cdot \cdot , 19\} \cup \{24, . . . , 27\}$
...
$$AB_{13,5} = \sum_{k=1}^{15} a_{13,k}b_{k5}$$
...
El primer sumando cumple que $a_{13,k} = 0$ $\forall k \in \{1, . . . , 5\}$ ya que recordemos, $A \in U_{15}$, con lo cual $a_{ij} = 0$ si, y solo si, $i > j$.

De forma similar, el segundo sumando cumple tanto que $a_{13,k} = 0$ $\forall k \in \{6, . . . , 12\}$ como que $b_{k5} = 0$ $\forall k \in \{6, . . . , 12\}$ porque en ambos casos $i > j$.
...
El primer sumando cumple que $a7,k = 0$ $\forall k \in \{1, . . . , 6\}$ ya que recordemos, $A \in U_{15}$, con lo cual $a_{ij} = 0$ si, y solo si, $i > j$.
...
Para calcular la entrada AB6,14, seguimos el razonamiento anterior: partimos la suma en 3 del siguiente
modo:
...
a_{6k}b_{k,14} +
\sum_{k=8}^{15}
a_{6k}b_{k,14} + a_{6,15}b_{15,14} =
\sum_{k=8}^{15}
a_{6k}b_{k,14}
$$
...
Por lo tanto, lo que queremos demostrar es que dadas $A, B \in U_n(\mathbb{K})$, el producto $AB \in Un(\mathbb{K})$.

Sean $i, j \in \{1, 2, . . . , n\}$ tales que $i > j$. Demostremos pues que las entradas $AB_{ij} = 0$.
...
Como las matrices $A$, $B$ son triangulares superiores, tenemos que

$A_{ik} = 0$ $\forall i, k \in \{1, 2, . . . , n\}$ tales que $i > k$

$B_{kj} = 0$ $\forall j, k \in \{1, 2, . . . , n\}$ tales que $k > j$
...
Donde el primer sumando vale $0$ porque $\forall k \in \{1, . . . , j\}, k  \leq j < i$; el segundo sumando es $0$ porque $\forall k \in \{j + 1, . . . , i - 1\}$, $j < k < i$; y, finalmente, el último sumando es nulo porque para todos los valores de $k, k \in \{i, . . . , n\}, j < i \leq k$. Así queda demostrado que $AB \in U_n$.

* En el caso de que $A, B \in Un(\mathbb{K})$, calcula la fórmula para las entradas diagonales $AB_{ii}$

* En el caso de que $A, B \in Un(\mathbb{K})$, calcula la fórmula para las entradas no triviales $AB_{ij}$ con $i \leq j$
